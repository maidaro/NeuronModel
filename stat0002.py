...
x_data = np.array([transform_xs(x) for x in data_learn.iloc[:,0].values]).reshape([-1,2])
y_data = cal_pyx(kernel_x_y1, kernel_x_y0, py, data_learn.iloc[:,0].values)
print(y_data)
test_data = df_raw.sample(n=1000)
x_test = np.array([transform_xs(x) for x in test_data.iloc[:,0].values]).reshape([-1,2])
...
x_summary = np.array([transform_xs(x) for x in xs]).reshape([-1,2])
